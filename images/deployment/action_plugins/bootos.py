...
class FabricProcess(multiprocessing.Process):
    def __init__(self, user, host, cmd, privileged):
        ...
        self.operation = (
            self.ctx.sudo if (user != "root" and privileged) else self.ctx.run
        )
        self.cmd = cmd
...
def run_cmd(host, user, cmd, privileged=False):
    logger.debug('run cmd "{}" on {}@{}'.format(cmd, user, host))
    ...
    if fp.exitcode is None:
        fp.terminate()
        logger.debug('timeout run cmd "{}" on {}@{}'.format(cmd, user, host))
        raise TimeoutError()

    logger.debug(
        'passed run cmd "{}" on {}@{}. Exit code: {}'.format(
...
class ActionModule(ActionBase):
    def run(self, tmp=None, task_vars=None):
        ...
        if not args["reboot"]:
            status = get_status(task_vars["ansible_host"], user)
            if status == args["image"]:
                result["changed"] = False
                return result

        if args["image"] == "base":
            self.boot_base(task_vars["ansible_host"], user)
            result["changed"] = True

        elif args["image"] == "experiment":
            res = self.boot_experiment(
                task_vars["ansible_host"], user, args["experimentuser"], args, task_vars
            )
            result.update(res)

        return result

    def boot_experiment(self, host, user, experimentuser, args, task_vars):
        ...
        res = self._execute_module(module_args=args, task_vars=task_vars)

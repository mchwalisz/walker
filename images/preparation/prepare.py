...
def __build(diskimage, release, kernel):
    ...
    sources_tmpl = jinja2.Environment(
        loader=jinja2.FileSystemLoader(str(pathlib.Path(__file__).parents[0]))
    ).get_template("sources.list.jn2")
    ...
    try:
        invoke.run(
            ...
            env={
                ...
                "DIB_RELEASE": release,
                "ELEMENTS_PATH": pathlib.Path(__file__).absolute().parents[0]
                / "add_elements",
            },
        )
    finally:
...
def __render_rspec(url):
    ...
    for node_type in node_spec:
        for node_name in node_spec[node_type]["hosts"]:
            node = {"name": node_name, "type": node_type, "disk_image": url}
            nodes.append(node)

    rspec = jinja2.Environment(
        loader=jinja2.FileSystemLoader(str(BASE_PATH / "node_selection"))
    ).get_template("rspec.jn2")
...
@click.option("--diskimage", "-d", default="image.tgz", help="Filename for diskimage")
@click.option("--release", "-r", default="xenial", help="Ubuntu release codename")
@click.option(
    "--kernel",
    "-k",
    multiple=True,
...
@cli.command()
@click.option("--url", help="diskimage URL to bake into rspec")
